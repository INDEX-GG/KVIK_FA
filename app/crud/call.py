...
import datetime
from app.db.db_models import PhoneCall, PhoneVerifyUnsuccessfulTry


def create_call(db: Session, phone: str, verification_code: str):
    db_call = PhoneCall(phone=phone,
                        phoneValidate=False,
...
def check_count_of_calls(db: Session, phone: str, max_count_of_calls_in_period: int = 3,
    ...
    db_calls = db.query(PhoneCall).filter(PhoneCall.phone == phone, PhoneCall.createdAt >= time_limit).all()
...
def check_verification_code(db: Session, phone: str, verification_code: str, time_period_in_minutes: int = 5):
    time_limit = datetime.datetime.utcnow() - datetime.timedelta(minutes=time_period_in_minutes)
    db_call = db.query(PhoneCall)\
        .filter(PhoneCall.phone == phone, PhoneCall.createdAt >= time_limit)\
        .order_by(PhoneCall.createdAt.desc()).first()
    if not db_call:
